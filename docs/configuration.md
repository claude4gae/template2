...
PM SPIDER는 단일 `/data/pm_spider` mount 아래에서 표준 `raw_data`/`score_data` 구조와 legacy `data`/`pm_spider_result` 구조를 모두 지원합니다. 둘 다 있으면 표준 `raw_data`/`score_data`를 우선 사용합니다.
