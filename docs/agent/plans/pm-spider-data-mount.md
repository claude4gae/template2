...
- `apps/api/api/pm_comparison/selectors.py`는 데이터 루트 바로 아래의 `data`, `result`를 조회한다.
- 실제 운영 데이터는 단일 mount 아래 `data`, `result` 이름으로 들어올 수 있다.
...
- PM Comparison selector는 `/data/pm_spider/data`와 `/data/pm_spider/result`만 해석한다.
...
- 대응: 운영/개발 실행 전 호스트 경로에 `data`, `result`가 있는지 확인하거나 `PM_COMPARISON_DATA_HOST_PATH`를 실제 위치로 지정한다.
...
- 2026-06-11: 호환 fallback을 제거하고 `/data/pm_spider/data`, `/data/pm_spider/result`만 지원하도록 selector와 테스트를 정리했다.
