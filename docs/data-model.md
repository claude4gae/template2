...
이 앱은 기본 업무 DB, timeline 전용 DB, MinIO, 외부 시스템을 함께 사용합니다.
...
| Timeline PostgreSQL | `TIMELINE_DB_*` | Timeline | line/SDWT/공정/설비/log 조회 |
...
| `api.timeline` | timeline 전용 DB read-only 조회 |
...
- Timeline은 기본 DB model 없이 별도 DB의 line/equipment/log 데이터를 selector에서 조회합니다.

## 변경 시 확인 항목

- migration이 필요한 기본 DB schema 변경인지 확인합니다.
- timeline DB는 외부/별도 schema이므로 Django migration 대상이 아닙니다.
