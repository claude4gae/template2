...
def _is_sop_delivery_eligible(row: dict[str, Any]) -> bool:
    """SOP가 delivery snapshot 생성 대상인지 확인합니다."""

    if _normalize_int_flag(row.get("instant_inform")) == 1:
        return True
    return _normalize_int_flag(row.get("needtosend")) == 1 and str(row.get("status") or "").strip() == "COMPLETE"


def _extract_sop_id(row: dict[str, Any]) -> int | None:
...
def _append_unique_target(*, target_list: list[str], target: str) -> None:
    ...
    if not target_key:
        return
    if any(_normalize_lookup_key(existing) == target_key for existing in target_list):
        return
...
def _normalize_channels(channels: Sequence[str]) -> list[str]:
    ...
    for channel in channels:
        if channel in _DELIVERY_CHANNELS and channel not in normalized:
