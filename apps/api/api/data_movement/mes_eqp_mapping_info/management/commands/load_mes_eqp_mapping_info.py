"""mes_eqp_mapping_info 파일 적재 management command입니다."""
...
from api.data_movement.mes_eqp_mapping_info import services


class Command(BaseCommand):
    """Airflow에서 호출할 mes_eqp_mapping_info 파일 적재 command입니다."""

    help = "Load mes_eqp_mapping_info deflate CSV files into PostgreSQL."
    ...
    def handle(self, *args, **options) -> None:
        """mes_eqp_mapping_info 파일 적재를 실행합니다."""

        data_dir = Path(options["data_dir"]) if options.get("data_dir") else None
        summary = services.load_mes_eqp_mapping_info_files(
            data_dir=data_dir,
        ...
        if summary.failure_count:
            raise CommandError(f"mes_eqp_mapping_info 적재 실패 파일 수: {summary.failure_count}")
