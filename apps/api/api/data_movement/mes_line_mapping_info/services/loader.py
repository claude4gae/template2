"""mes_line_mapping_info 파일 적재 서비스입니다."""
...
class LoadRunSummary:
    """mes_line_mapping_info 적재 실행 요약입니다."""
    ...
    def success_count(self) -> int:
        ...
        return sum(1 for outcome in self.outcomes if outcome.status == MesLineMappingInfoLoadJob.Status.SUCCESS)

    @property
    def failure_count(self) -> int:
        """실패한 파일 수를 반환합니다."""

        return sum(1 for outcome in self.outcomes if outcome.status == MesLineMappingInfoLoadJob.Status.FAILED)


def _finish_job(
    *,
    job: MesLineMappingInfoLoadJob,
    status: str,
    row_count: int,
    error_message: str | None = None,
) -> None:
...
def _create_job(*, claimed_file: ClaimedDataFile) -> MesLineMappingInfoLoadJob:
    """선점한 파일 기준으로 적재 이력 row를 생성합니다."""

    return MesLineMappingInfoLoadJob.objects.create(
        file_name=claimed_file.original_name,
        file_path=str(claimed_file.original_path),
        status=MesLineMappingInfoLoadJob.Status.RUNNING,
...
def _load_claimed_file(*, claimed_file: ClaimedDataFile) -> LoadFileOutcome:
    ...
    try:
        ...
        _finish_job(
            job=job,
            status=MesLineMappingInfoLoadJob.Status.SUCCESS,
...
def _dry_run_one_file(*, file_path: Path) -> LoadFileOutcome:
    ...
    job = MesLineMappingInfoLoadJob.objects.create(
    ...
    try:
        ...
        _finish_job(
            job=job,
            status=MesLineMappingInfoLoadJob.Status.DRY_RUN,
            row_count=row_count,
        )
        return LoadFileOutcome(
            file_name=file_path.name,
