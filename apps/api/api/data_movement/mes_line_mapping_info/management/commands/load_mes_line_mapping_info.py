...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options) -> None:
        """mes_line_mapping_info 파일 적재를 실행합니다."""

        data_dir = Path(options["data_dir"]) if options.get("data_dir") else None
        summary = services.load_mes_line_mapping_info_files(
            data_dir=data_dir,
        ...
        if summary.failure_count:
            raise CommandError(f"mes_line_mapping_info 적재 실패 파일 수: {summary.failure_count}")
