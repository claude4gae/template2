...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options) -> None:
        """ctttm_workorder_list 파일 적재를 실행합니다."""

        data_dir = Path(options["data_dir"]) if options.get("data_dir") else None
        summary = services.load_ctttm_workorder_list_files(
            data_dir=data_dir,
        ...
        for outcome in summary.outcomes:
            message = (
                f"{outcome.status}: {outcome.file_name}, "
                f"source={outcome.source_type or '-'}, rows={outcome.row_count}"
            )
        ...
        if summary.failure_count:
            raise CommandError(f"ctttm_workorder_list 적재 실패 파일 수: {summary.failure_count}")
