...
class AccountEndpointTests(TestCase):
    ...
    def test_dev_dummy_affiliation_get_exposes_temporary_option(self) -> None:
        """dev 더미 사용자는 소속 선택 목록에서 임시 소속을 볼 수 있어야 합니다."""
        ...
        self.assertIn(
            {
                ...
                "line": "DEV",
                "user_sdwt_prod": "DEV_DUMMY_USER_SDWT",
            },
            payload["affiliationOptions"],
        )
        self.assertEqual(payload["snapshotUserSdwtProd"], "DEV_DUMMY_USER_SDWT")
        self.assertFalse(Affiliation.objects.filter(user_sdwt_prod="DEV_DUMMY_USER_SDWT").exists())

    @override_settings(DEBUG=True)
    def test_dev_dummy_affiliation_post_applies_temporary_option(self) -> None:
        """dev 더미 사용자가 임시 소속을 선택하면 즉시 현재 소속으로 적용되어야 합니다."""
        ...
        response = self.client.post(
            reverse("account-affiliation"),
            data='{"userSdwtProd":"DEV_DUMMY_USER_SDWT"}',
            content_type="application/json",
        )

        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["status"], "applied")
        self.assertEqual(get_current_user_sdwt_prod(user=dummy_user), "DEV_DUMMY_USER_SDWT")
        option = Affiliation.objects.get(user_sdwt_prod="DEV_DUMMY_USER_SDWT")
        self.assertEqual(option.department, "Development")
        self.assertEqual(option.line, "DEV")

    @override_settings(DEBUG=True)
    def test_dev_dummy_line_sdwt_options_include_temporary_option(self) -> None:
        """dev 더미 사용자는 line/user_sdwt_prod 옵션에서도 임시 소속을 받을 수 있어야 합니다."""
        ...
        payload = response.json()
        self.assertIn(
            {"lineId": "DEV", "userSdwtProds": ["DEV_DUMMY_USER_SDWT"]},
            payload["lines"],
        )
        self.assertIn("DEV_DUMMY_USER_SDWT", payload["userSdwtProds"])

    @override_settings(DEBUG=False)
    def test_dev_dummy_affiliation_option_is_limited_to_debug(self) -> None:
        """DEBUG가 꺼져 있으면 더미 사용자 임시 소속을 노출하지 않아야 합니다."""
